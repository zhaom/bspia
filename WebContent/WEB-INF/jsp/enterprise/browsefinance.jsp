...
<div> 
	<table class="basic-table-sh">
		...
		<tr>
			<td colspan="3">
				<form id="editForm" class="submitForm" action="/enterprise/browsefinance.action" method="post">
					<table>
						<tr>
  ...
  <s:action name="enterprise!listfinance" namespace="/enterprise" executeResult="true" >
  	<s:param name="pno" value="#parameters.pno"/>
    <s:param name="rows" value="10"/>
    <s:param name="requestId" value="#parameters.requestId"/>
  </s:action>
   <div class="buttonUl"><ul>
  	<m:pager curPage="%{#parameters.pno[0]}" totalPage="%{#paging.totalPage}" theme="simple" url="/enterprise/browsefinance.action?pno={page}&requestId=%{#request.encodeRequestId}" pageLimit="10" showTotalPage="false" totalRecord="%{#paging.totalRecord}" showTotalRecord="false" />
  	</ul></div>