...
<div> 
	<table class="basic-table-sh">
		...
		<tr>
			<td>
				<form id="editForm" class="submitForm" action="/correspondent/browseExcellentSelect.action" method="post">
					<table>
						<tr>
  ...
  <s:action name="excellentselect!list" namespace="/correspondent" executeResult="true" >
  	<s:param name="pno" value="#parameters.pno"/>
    <s:param name="rows" value="10"/>
    <s:param name="requestId" value="#parameters.requestId"/>
  </s:action>
	<div class="buttonUl"><ul><m:pager curPage="%{#parameters.pno[0]}" totalPage="%{#paging.totalPage}" theme="simple" url="/correspondent/browseExcellentSelect.action?pno={page}&requestId=%{#request.encodeRequestId}" pageLimit="5" showTotalPage="false" totalRecord="%{#paging.totalRecord}" showTotalRecord="false" /></ul></div>
  <div class="buttonUl"><ul><li><a class="submitButton" href="/correspondent/excellentselect!add.action"><s:property value="%{getText('common_add')}"/></a></li></ul></div>